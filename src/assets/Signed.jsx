...
import { 
  ...
  Award, CheckCircle, Sparkles, GraduationCap, Globe, LogOut, Loader2, AlertCircle
} from 'lucide-react';
import { onAuthStateChanged, signOut } from "firebase/auth";
import { auth } from "../assets/firebase"; // Ensure this points to your firebase config
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        // If no user is logged in, redirect them back to auth
        navigate("/auth"); 
      }
      setLoading(false);
    });
        ...
        <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-white/70 dark:bg-[#050505]/70 backdrop-blur-2xl border-b border-black/5 dark:border-white/5 py-3' : 'bg-transparent border-transparent py-5'}`}>
          <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              {/* FIXED LOGO BACKGROUND: Adapts to light/dark so black/white icon is visible */}
            ...
            <div className="flex items-center gap-4">
              ...
              {/* User Name */}
              <div className="hidden sm:block text-slate-700 dark:text-slate-300 text-sm font-medium pr-2">
        ...
        <main className="flex-1 relative z-10 pt-32">
          ...
          <section className="py-32 px-6 relative overflow-hidden">
            <div className="max-w-5xl mx-auto rounded-[40px] p-12 lg:p-20 text-center relative overflow-hidden shadow-2xl border border-white/10 group">
              ...
              <h2 className="text-4xl lg:text-6xl font-bold text-white tracking-tighter mb-6 relative z-10">Access your Workspace</h2>
              <p className="text-indigo-100 text-lg md:text-xl mb-12 max-w-2xl mx-auto relative z-10 font-light">
                Jump right back into your active study sessions, pick up where you left off, and check your pending tasks.
              </p>
          ...
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            <div className="md:col-span-2">
              <div className="flex items-center gap-3 mb-6">
                {/* FIXED FOOTER LOGO BACKGROUND */}
                <div className="bg-slate-200 dark:bg-white/10 border border-slate-300 dark:border-white/10 p-2 rounded-lg transition-colors">
                 <GraduationCap className="text-black dark:text-white" size={24} />
                </div>
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm text-slate-500 dark:text-slate-400">
                ...
                <li><a href="#" className="hover:text-slate-900 dark:hover:text-white transition-colors">Courses</a></li>
                <li><a href="#" className="hover:text-slate-900 dark:hover:text-white transition-colors">Study Groups</a></li>
                <li><a href="#" className="hover:text-slate-900 dark:hover:text-white transition-colors">Settings</a></li>
              </ul>
            </div>
